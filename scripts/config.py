...
def create_starter_script(user, is_master):
    ...
    template = [
        ...
        'rosrun distributed_systemd_start distributed_ros_{}'.format('master' if is_master else 'slave'),
    ]
    return '\n'.join(template)


def create_systemd_script(iface):
    """
    ...
    template = [
        ...
        'ExecStart={}/distributed_ros'.format(ROS_BIN),
        'Restart=on-abort',
        '[Install]',
...
if args.clean:
    ...
    if os.path.exists('/etc/init/distributed-ros.conf'):
        command = "sudo rm /etc/systemd/system/distributed-ros.service"
...
if check_settings():
    ...
    # write systemd service file (utilizing sudo)
    systemd_script = create_systemd_script(iface)
    try:
        command = "echo '{}' > /etc/systemd/system/distributed-ros.service".format(systemd_script)
        subprocess.call(["/usr/bin/sudo", "sh", "-c", command])
    except:
        raise
    ...
    try:
        command = "systemctl enable distributed-ros.service"
        subprocess.call(["/usr/bin/sudo", "sh", "-c", command])
    except:
        raise
    print ' enable Systemd Service "distributed-ros.service".'
